...
void validate_result(float *device_result, const float *cpu_reference, const char *name, std::size_t num_elements, float tolerance = 1e-4) {
    float *out_gpu = (float *)malloc(num_elements * sizeof(float));
    cudaCheck(cudaMemcpy(out_gpu, device_result, 1 * sizeof(float), cudaMemcpyDeviceToHost));
    int nfaults = 0;
